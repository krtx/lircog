...
(register-command privmsg (time channel _ nick message)
 ...
 (:div :class "container-fluid"
       ...
       (:div :class "span1 nick" (str nick))
       (:div :class "span5" (esc message))))
...
(defun information (time info)
  `(:div :class "container-fluid"
         (:div :class "span1 time" (str ,time))
         (:div :class "span6" (esc ,info))))
...
(register-command join (time nick info channel)
  "^(\\d{2}:\\d{2}:\\d{2}) \\+ ([^\\s]+) \\(([^)]+)\\) to (.+)$"
  (information time (format nil "+ ~A (~A)" nick info)))
  ;; (:div :class "container-fluid"
  ;;       (:div :class "span1 time" (str time))
  ;;       (:div :class "span6" (esc (format nil "+ ~A (~A)" nick info)))))
...
(register-command nick (time _ nick new-nick)
  ...
  (:div :class "container-fluid"
        (:div :class "span1 time" (str time))
        (:div :class "span6"
              (esc (format nil "~A -> ~A" nick new-nick)))))
...
(register-command topic (time channel nick topic)
  ...
  (:div :class "container-fluid"
        (:div :class "span1 time" (str time))
        (:div :class "span6"
              (esc (format nil "~A set topic ~A" nick topic)))))
